...
const UNISWAP_ROUTER_ABI = [ /* ... */ ]; // ABI of the Uniswap V2 Router contract
const WETH_ABI = [ /* ... */ ]; // ABI of the WETH contract

const ethToken = new Token(ChainId.MAINNET, '0x0000000000000000000000000000000000000000', 18);
...
const ETHUSDSwapSection = ({ web3, account }) => {
  ...
  useEffect(() => {
    const fetchTokenPrices = async () => {
      try {
        ...
        const trade = new Trade(route, new TokenAmount(ethToken, '1000000000000000000'), TradeType.EXACT_INPUT);
  ...
  const handleSwap = async () => {
    try {
      ...
      const weth = new web3.eth.Contract(WETH_ABI, WETH[ChainId.MAINNET].address);
        ...
        <div className="row">
          ...
          <div className="col-md-10">
            <div className="form-grp">
              <input type="number" id="inputAmount" value={inputAmount} onChange={e => setInputAmount(e.target.value)} />
            </div>
          </div>
          ...
          <div className="col-md-10">
            <div className="form-grp">
              <input type="number" id="outputAmount" value={outputAmount} readOnly />
            </div>
          </div>

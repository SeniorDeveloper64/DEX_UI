...
const ConnectWalletButton = () => {
  ...
  const handleConnect = async () => {
    ...
    if (typeof window.ethereum !== 'undefined') {
      ...
      const usdtContractAbi = [{"constant":true,"inputs":[{"name":"_owner","type":"address"}],"name":"balanceOf","outputs":[{"name":"","type":"uint256"}],"payable":false,"stateMutability":"view","type":"function"}]; // USDT contract ABI
      const usdtContract = new web3.eth.Contract(usdtContractAbi, usdtContractAddr);
      const balanceInWeiUSDT = await usdtContract.methods.balanceOf(account[0]).call();
